...
# Original: https://raw.githubusercontent.com/gotansible/runit/master/library/runit.py
# Licence: https://github.com/gotansible/runit/blob/master/LICENSE
...
def main():
    ...
    service_main_log_dir = service_log_dir + '/main'
    service_main_log_dir_args = module.load_file_common_arguments(dict(path=service_log_dir))
    service_main_log_dir_args['mode'] = 0755
    if get_file_state(service_main_log_dir) != 'directory':
        changed |= directory(module, service_main_log_dir, service_main_log_dir_args)
    ...
    service_main_log_configuration_file = service_main_log_dir + '/config'
    changed |= write_file(module, '\n'.join(log_configuration), service_main_log_configuration_file)

    log_command = '''#!/bin/sh
exec svlogd -tt %s
    ''' % (service_main_log_dir)
    ...
    command_text = '''#!/bin/sh
exec 2>&1
# Don't set fire to the CPU if we can't start
sleep 1
...
exec chpst -e /etc/sv/%s/env -u %s %s
  ''' % ('\n'.join(command_setup), name, runas, command)
    ...
    module.exit_json(status=status,restarted=restarted, changed=changed, run_service_file=run_service_file, log_service_file=log_service_file, service_dir=service_dir, enabled_service_dir=enabled_service_dir, enabled_state=enabled_state, enabled=enabled)
